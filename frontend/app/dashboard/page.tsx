...
import { Search, ChevronDown, Hexagon, LogOut, Loader2, RefreshCw } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const fetchData = async () => {
    ...
    const companyId = user.companyId || 1; // Fallback to 1 if not defined
  ...
  // Helper: Format Role Name for Badge
  ...
  return (
    <div className="min-h-screen bg-background text-primary-text font-sans">
      {/* 1. HEADER */}
      <header className="sticky top-0 z-30 bg-surface border-b border-border h-16 px-6 lg:px-8 flex items-center justify-between shadow-xs">
        <div className="flex items-center space-x-2">
          <Hexagon className="h-6 w-6 text-primary" strokeWidth={2.5} />
          <span className="text-xl font-bold tracking-tight text-primary-text">TransitOps</span>
        </div>

        {/* Search Bar */}
        <div className="relative w-full max-w-md mx-8 hidden sm:block">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-secondary-text/70" />
          <input
            type="text"
            placeholder="Search trips, vehicles, drivers..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full h-10 pl-10 pr-4 bg-background border border-border rounded-md text-[14px] placeholder:text-secondary-text/50 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary/30 transition-all"
          />
        </div>
        ...
        <div className="flex items-center space-x-4">
          <button
            ...
            className="p-2 hover:bg-background rounded-full transition-colors text-secondary-text"
          >
          ...
          <div className="flex items-center space-x-3">
            <div className="hidden md:flex flex-col text-right">
              <span className="text-[14px] font-semibold leading-tight text-primary-text">
                {user.name}
              </span>
              <span className="text-[11px] font-medium text-secondary-text bg-background border border-border/80 px-1.5 py-0.5 rounded mt-0.5">
                {formatRole(user.role)}
              </span>
            </div>

            <div className="h-9 w-9 rounded-full bg-primary text-white font-semibold flex items-center justify-center text-[14px] shadow-xs border border-primary/20">
            ...
            <button
              ...
              title="Sign Out"
              className="p-2 hover:bg-error/10 hover:text-error rounded-full transition-all text-secondary-text"
            >
      ...
      <main className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
        ...
        <div className="relative w-full block sm:hidden">
          ...
          <input
            ...
            placeholder="Search trips, vehicles, drivers..."
            value={searchQuery}
            className="w-full h-10 pl-10 pr-4 bg-surface border border-border rounded-md text-[14px]"
            onChange={(e) => setSearchQuery(e.target.value)}
          />
        </div>
        ...
        <div className="bg-surface border border-border p-4 rounded-lg flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="text-[14px] font-semibold text-secondary-text uppercase tracking-wider">
            Filters
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:w-auto w-full">
            ...
            <div className="relative">
              <select
                ...
                className="w-full md:w-44 h-10 rounded-md bg-background border border-border px-3 pr-8 text-[13px] font-medium text-primary-text focus:outline-none focus:ring-1 focus:ring-primary cursor-pointer appearance-none"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-surface border border-border rounded-lg shadow-xs lg:col-span-2 flex flex-col">
            <div className="px-5 py-4 border-b border-border flex items-center justify-between">
              ...
              {loading && <Loader2 className="h-4 w-4 animate-spin text-secondary-text" />}
            </div>
                ...
                <div className="space-y-5 flex-1 flex flex-col justify-center">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-[13px] font-semibold">
                      ...
                      <span className="text-primary-text">
                        {kpiData.vehicles.inMaintenance} (
                        {getPercent(kpiData.vehicles.inMaintenance)}%)
                      </span>
                    </div>
                    <div className="h-3 bg-background rounded-full overflow-hidden border border-border/80">
                      <div
