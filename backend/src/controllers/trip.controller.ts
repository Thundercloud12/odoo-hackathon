...
export const getTripById = async (req: Request<{ id: string }>, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
export const createTrip = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!reg_no || !driver_id || !src || !dest || cargo_weight === undefined || trip_dist === undefined) {
      return res.status(400).json({ success: false, message: 'All trip fields (reg_no, driver_id, src, dest, cargo_weight, trip_dist) are required' });
    }
    ...
    if (error.message && (error.message.includes('Vehicle') || error.message.includes('Driver') || error.message.includes('Cargo'))) {
...
export const updateTripStatus = async (req: Request<{ id: string }>, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    ...
    const updatedTrip = await prisma.$transaction(async (tx) => {
      ...
      if (trip_status === 'Dispatched') {
        ...
        await tx.driver.update({ where: { driver_id: trip.driver_id }, data: { status: 'On_Trip' } });
        ...
        await tx.driver.update({ where: { driver_id: trip.driver_id }, data: { status: 'Available' } });
      }
      return updated;
    });
